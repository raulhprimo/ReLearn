// Script de seed usando better-sqlite3 diretamente
import Database from 'better-sqlite3'
import { fileURLToPath } from 'url'
import { dirname, join } from 'path'

const __dirname = dirname(fileURLToPath(import.meta.url))
const dbPath = join(__dirname, '..', 'data', 'raul.db')
...
const insertArea = db.prepare(`
  INSERT OR IGNORE INTO areas (codigo, nome, nivel, cor, icone, ativa)
  VALUES (@codigo, @nome, @nivel, @cor, @icone, 1)
`)

const insertTopico = db.prepare(`
  INSERT OR IGNORE INTO topicos (area_id, nome, ordem, status)
  VALUES (@areaId, @nome, @ordem, 'nao_iniciado')
`)

const getArea = db.prepare(`SELECT id FROM areas WHERE codigo = ?`)
...
for (const area of AREAS) {
  insertArea.run(area)
  const row = getArea.get(area.codigo)
  const areaId = row.id
  for (let i = 0; i < area.topicos.length; i++) {
    insertTopico.run({ areaId, nome: area.topicos[i], ordem: i })
    totalTopicos++
  }
  totalAreas++
}

console.log(`✓ Seed concluído: ${totalAreas} áreas, ${totalTopicos} tópicos`)
db.close()
